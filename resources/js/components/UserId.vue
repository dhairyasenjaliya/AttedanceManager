...
export default {
   components: { VueClock , Datepicker },
        ...
        methods:{  
                ...
                week() {
                               axios.get("/api/week?id="+this.id).then(({ data }) => {  
                  ...
                  this.$Progress.start();
                    ...
                    fetchtimsheet() {
                      if(this.$gate.isAdmin()){
                              //  axios.get("/api/userid?id= & date= " + this.id + moment( this.state.date).format('YYYY-MM-DD')).then(({ data }) => { 
